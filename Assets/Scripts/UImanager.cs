...
public class UImanager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        chestSequence.Join(transform.DORotate(Vector3.up * 0, 0.50f));
        //third animation = shake (duration, strength, vibrato, randomness) & scaling
        chestSequence.Append(transform.DOShakeRotation(1f, 25f, 30, 10f));
        ...
        chestSequence.Join(transform.DOScaleY(transform.localScale.y / 1f, 0.50f));
        chestSequence.Join(transform.DOScaleX(transform.localScale.x / 1f, 0.50f));
        chestSequence.Join(transform.DOScaleZ(transform.localScale.z / 1f, 0.50f));
    }
